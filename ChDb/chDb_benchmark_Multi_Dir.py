"""chDB Partitioned Benchmark

Evaluates chDB query performance on Hive-partitioned Parquet files.
Partitioning enables automatic partition pruning for optimized queries.
"""
...
# Convert to absolute path for chDB
TRIPS_PATH_ABS = os.path.abspath(TRIPS_PATH)

# Data source for chDB queries - wildcard pattern for partitioned directories
CHDB_SOURCE = f"file('{TRIPS_PATH_ABS}/**/*.parquet', Parquet)"

# Queries
CHDB_QUERIES = {
    "Q1_Count":              f"SELECT count() FROM {CHDB_SOURCE}",
    "Q2_SumAvg":             f"SELECT sum(total_amount), avg(trip_distance) FROM {CHDB_SOURCE}",
    "Q3_YearAgg":            f"SELECT year, count() AS trips, sum(total_amount) AS revenue FROM {CHDB_SOURCE} GROUP BY year ORDER BY year",
    ...
    "Q6_TimeRange":          f"SELECT count() FROM {CHDB_SOURCE} WHERE year = '2020' AND month = '6'",
